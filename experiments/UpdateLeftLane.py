    ...
    for cp in checkpoints:
        ...
        if len(white_pixels) == 0:
            ...
            if max_x - min_x >= image.shape[1] - 1:
                ...
                if Config.LANE_MODE == "LEFT":
                    # chạy lane phải → bias về rìa phải
                    center = int(max_x * Config.CENTER_BIAS + (1 - Config.CENTER_BIAS) * min_x)
                else:
                    # chạy lane trái → bias về rìa trái
                    center = int(min_x * Config.CENTER_BIAS + (1 - Config.CENTER_BIAS) * max_x)
...
def calculate_weighted_slope(center_positions, image_width, image_height):
    ...
    for center, checkpoint, weight in zip(center_positions, 
                                          [Config.UPPER_CHECKPOINT, Config.MID_CHECKPOINT, Config.LOWER_CHECKPOINT], 
                                          weights):
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            print(f"[{Config.LANE_MODE}] PID adjusted angle: {pid_adjusted_angle:.2f} degrees")
